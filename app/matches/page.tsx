...
export default function MatchesPage() {
  ...
  const calculateStats = (matchesData: any[]) => {
    ...
    setStats({
      ...
      commonInterests: Math.round(
        matchesData.reduce((acc, m) => acc + (m.breakdown?.interests || 0), 0) / 
        matchesData.length * 100
      ),
      avgScore: Math.round(
        matchesData.reduce((acc, m) => acc + m.compatibilityScore, 0) / 
        matchesData.length
      )
    })
  }
        ...
        <div className="mb-12">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500">
              Encontramos {realDataCount} personas compatibles contigo
            </p>
          </div>
                ...
                <div className="p-5 pb-0">
                  <div className="flex justify-between items-start mb-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                      objectiveMap[objKey]?.color || 'text-gray-700 bg-gray-100'
                    }`}>
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mb-6 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
                        score > 75 ? 'bg-green-500' : 'bg-blue-500'
                      }`}
                      style={{ width: `${score}%` }}
                    ></div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500 truncate flex items-center gap-1">
                        {userData.age ? `${userData.age} años` : 'Estudiante'} • 
                        {userData.city?.split(',')[0] || 'Campus'}
                      </p>
                    </div>
                  </div>
